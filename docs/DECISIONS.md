...
| D-004 | proposed | Git-to-Supabase sync는 Git commit 이후 별도 GitHub Actions workflow로 실행한다. |
| D-005 | proposed | Web은 `apps/web`, importer는 별도 package, DB migration은 `supabase`에 격리한다. |
| D-006 | proposed | Next.js App Router의 Server Component 우선 구조를 사용한다. |
| D-007 | confirmed | 사용자 반응은 사실 신뢰도와 출처 검증 상태를 변경하지 않는다. |
| D-008 | proposed | Source authority는 source에, verification status는 event-source 관계에 저장한다. |
| D-009 | proposed | 명시적 AI 점수가 없는 legacy Event에는 임의 점수를 만들지 않는다. |
| D-010 | proposed | 사용자 reaction은 Event별 단일 row에 sentiment와 interested를 분리한다. |
| D-011 | confirmed | 결제, entitlement, 댓글, 컬렉션, 추천, 알림, 커뮤니티는 V1에서 제외한다. |
| D-012 | proposed | Supabase write는 service-role 전용 atomic RPC로 제한한다. |
| D-013 | proposed | 외부 이미지는 명시적 출처가 있을 때만 사용하고 무단 복제를 하지 않는다. |
| D-014 | proposed | Opportunity–Event 관계는 명시적 근거 키가 있을 때만 생성한다. |
| D-015 | proposed | Build-candidate 상태는 Opportunity 전역이 아니라 Briefing occurrence에 저장한다. |
| D-016 | proposed | Legacy analysis 원문을 항상 보존하고 명확한 label만 구조화한다. |
| D-017 | proposed | 공개 Event route는 내부 `event_key`가 아니라 별도 `slug`를 사용한다. |
...
- 상태: `proposed`
- 결정: 현재 Git에 명시적 수치 점수가 없으면 `ai_score = null`로 저장하고 UI에 미제공 상태를 표시한다.
- 근거: 중요도 S/A/B를 임의 수치로 매핑하면 실제 평가 근거를 왜곡한다.
- 고려한 대안:
  - S/A/B를 고정 점수로 변환
  - Web sync에서 새 모델로 재평가
- 이유: 출처가 없는 정밀도를 만들지 않는다.
- 영향: V1 Detail은 점수 fallback UI가 필요하다.
- 재검토 조건: Git schema에 score와 method version이 명시적으로 추가될 때
...
- 영향: 향후 기능은 별도 승인과 migration으로 추가한다.
...
- 상태: `proposed`
- 결정: 명시적 Source image와 YouTube thumbnail을 우선하고, 권리 확인 없는 외부 이미지를 Storage로 복제하지 않는다.
- 근거: V1은 text-first이며 대표 이미지는 보조 요소다.
- 고려한 대안: 모든 Source의 Open Graph image 자동 복제
- 이유: 저작권, 저장 비용, broken image와 hotlink 위험을 줄인다.
- 영향: 이미지 없는 Event를 위한 일관된 fallback이 필요하다.
...
1. 로그인 provider 범위
2. legacy 콘텐츠 한국어 번역 방식
3. AI 평가 점수를 V1 필수 표시로 볼지 여부
4. 대표 이미지 수집과 저장 정책
5. Source tier → authority/verification 매핑표
6. `event_key` 전역 identity 계약
7. `partial` briefing 공개 규칙
8. Supabase/Vercel 리전과 비용 한도
