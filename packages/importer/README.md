...
`incremental`과 `backfill` write mode에는 server-side `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`가 필요하다. GitHub main workflow는 누락된 pending run도 최신 snapshot이 흡수하도록 `backfill`을 사용한다. 서비스 키는 마지막 sync step에만 주입되며 브라우저 bundle이나 dry-run에는 전달하지 않는다.
