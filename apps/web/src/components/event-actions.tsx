...
export function EventActions({ eventId, returnPath }: { eventId: string; returnPath: string }) {
  ...
  return <div><div className="grid grid-cols-5 divide-x divide-slate-200 overflow-hidden rounded-lg border border-slate-200 bg-white">
    <Button aria-pressed={sentiment === "like"} variant="ghost" size="sm" className="h-12 rounded-none px-1 text-[11px] sm:text-xs" disabled={pending} onClick={() => writeReaction(sentiment === "like" ? null : "like", interested)}><ThumbsUp aria-hidden />좋아요</Button>
    <Button aria-pressed={sentiment === "dislike"} variant="ghost" size="sm" className="h-12 rounded-none px-1 text-[11px] sm:text-xs" disabled={pending} onClick={() => writeReaction(sentiment === "dislike" ? null : "dislike", interested)}><ThumbsDown aria-hidden />싫어요</Button>
    <Button aria-pressed={interested} variant="ghost" size="sm" className="h-12 rounded-none px-1 text-[11px] sm:text-xs" disabled={pending} onClick={() => writeReaction(sentiment, !interested)}><Heart aria-hidden />관심</Button>
    <Button aria-pressed={bookmarked} variant="ghost" size="sm" className="h-12 rounded-none px-1 text-[11px] sm:text-xs" disabled={pending} onClick={toggleBookmark}><Bookmark aria-hidden />저장</Button>
    <Button variant="ghost" size="sm" className="h-12 rounded-none px-1 text-[11px] sm:text-xs" onClick={share}><Share2 aria-hidden />공유</Button>
  </div>{message && <p role="status" className="mt-2 text-xs text-slate-600">{message}</p>}<p className="sr-only">개인 기능을 사용할 때만 로그인하며, 사용자 반응은 사실 신뢰도에 영향을 주지 않습니다.</p></div>;
}
