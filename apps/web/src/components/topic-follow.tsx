...
export function TopicFollow({ names, returnPath }: { names: string[]; returnPath: string }) {
  ...
  return <div className="mt-6"><p className="text-xs font-bold text-slate-500">TOPIC FOLLOW</p><div className="mt-2 flex flex-wrap gap-2">{(topics.length ? topics : names.map((name) => ({ id: "", name_ko: name }))).map((topic) => <Button key={topic.id || topic.name_ko} variant={followed.has(topic.id) ? "default" : "outline"} size="sm" disabled={(configured && !authResolved) || Boolean(userId && !topic.id)} onClick={() => toggle(topic)}>{followed.has(topic.id) ? <X aria-hidden /> : <Plus aria-hidden />}{topic.name_ko}</Button>)}</div></div>;
}
