...
export function OriginalContent({ content }: { content: OriginalContentDto }) {
  const [expanded, setExpanded] = useState(false);
  const contentId = useId();

  if (content.mode === "unavailable") return <section aria-labelledby={`${contentId}-title`} className="detail-section">
    <div className="detail-heading"><span>1.</span><h2 id={`${contentId}-title`}>원문 기반 상세 내용</h2><span className="content-mode">원문 내용 미확보</span></div>
  ...
  return <section aria-labelledby={`${contentId}-title`} className="detail-section">
    <div className="detail-heading"><span>1.</span><h2 id={`${contentId}-title`}>원문 기반 상세 내용</h2><span className="content-mode">{content.label}</span></div>
    <p className="mt-2 text-xs leading-5 text-slate-500">외부 기사의 전문 번역이 아니라, 보유한 출처 기반 사실을 재구성한 상세 요약입니다.</p>
